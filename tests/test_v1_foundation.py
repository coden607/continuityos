...
def test_routing_learner_uses_observed_success(tmp_path: Path):
    ...
    for _ in range(4):
        tel.record("a", "code", success=True, latency_ms=20, tokens=10, cost=0)
    for _ in range(4):
        tel.record("b", "code", success=False, latency_ms=20, tokens=10, cost=0)
...
def test_mem0_adapter_with_injected_client():
    class Client:
        ...
        def add(self, value, user_id, metadata):
            self.rows.append((value, user_id, metadata))

        def search(self, query, user_id, limit):
            return {"results": [{"memory": "hello", "metadata": {"key": "k"}}]}

    c = Client()
    adapter = Mem0Adapter(c)
...
def test_mcp_convenience_methods(monkeypatch):
    client = MCPClient()
    server = MCPServer(name="x", url="https://example.com")
    calls = []
